<template>
  ...
  <div ref="modal" v-if="generalStore.currentModal === 'shareModal'"
    ...
    <div class="bg-white relative rounded-lg p-6 w-full max-w-md">
      ...
      <button @click="generalStore.closeModal()" class="text-gray-600 hover:text-gray-900 absolute top-0 right-0 p-2">
        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path>
        </svg>
      </button>
      <div class="flex justify-between mb-5">
        <font-awesome-icon class="text-[#245567] text-3xl py-6 px-6 bg-[#a1e1f8] rounded-3xl"
          :icon="['fab', 'x-twitter']" />
        <font-awesome-icon class="text-blue-600 text-3xl py-6 px-6 bg-[#a1e1f8] rounded-3xl"
          :icon="['fab', 'facebook-f']" />
        <font-awesome-icon class="text-pink-600 text-3xl py-6 px-6 bg-[#fdb4cc] rounded-3xl"
          :icon="['fab', 'instagram']" />
        <font-awesome-icon class="text-blue-700 text-3xl py-6 px-6 bg-[#a1e1f8] rounded-3xl"
          :icon="['fab', 'linkedin']" />
      </div>
      ...
      <div class="flex items-center justify-between p-4 rounded mb-4">
        ...
        <button id="copyButton" class="text-gray-600 ml-2 focus:outline-none" onclick="copyToClipboard()">
  ...
  <div ref="modal" v-if="generalStore.currentModal === 'filterModal'"
    class="fixed inset-0 flex items-end justify-end z-50 transition-transform duration-300 transform" :class="{
      ...
      <div class="modal-content">
        ...
        <div class="mb-4">
          ...
          <input
            ...
            type="date" v-model="generalStore.filterParams.project_start_at" />
        </div>
        <div class="mb-4">
          <label class="block text-sm font-medium text-gray-700">End At</label>
          <input
            class="border-b w-60 border-gray-300 rounded-md py-2 pl-3 pr-4 focus:outline-none focus:border-blue-500"
            type="date" v-model="generalStore.filterParams.project_end_at" />
        </div>
        <div class="mb-4">
          <label class="block text-sm font-medium text-gray-700">Project Budget</label>
          <input
            class="border-b w-60 border-gray-300 rounded-md py-2 pl-3 pr-4 focus:outline-none focus:border-blue-500"
            type="number" v-model="generalStore.filterParams.project_budget" />
        </div>
        <div class="mb-4">
          <label class="block text-sm font-medium text-gray-700">Department</label>
          <select
            class="border-b w-60 border-gray-300 rounded-md py-2 pl-3 pr-4 focus:outline-none focus:border-blue-500"
            name="status" v-model="generalStore.filterParams.department_id">
            <option v-for="(department, index) in generalStore.data.filter_form.departments" :key="index"
              :value="department.id">
        ...
        <div class="mb-4">
          ...
          <select
            class="border-b w-60 border-gray-300 rounded-md py-2 pl-3 pr-4 focus:outline-none focus:border-blue-500"
            name="status" v-model="generalStore.filterParams.project_stage">
            <option v-for="(stage, index) in generalStore.data.filter_form.stages" :key="index" :value="index">
              {{ stage }}
            </option>
          </select>
        </div>
        <div class="mb-4">
          ...
          <select
            class="border-b w-60 border-gray-300 rounded-md py-2 pl-3 pr-4 focus:outline-none focus:border-blue-500"
            name="status" v-model="generalStore.filterParams.project_priority">
            <option class="bg-white text-black rounded-md"
              v-for="(priority, index) in generalStore.data.filter_form.priorities" :key="index" :value="index">
              {{ priority }}
            </option>
          </select>
        </div>
        ...
        <div class="mb-4">
          ...
          <div class="flex -space-x-2">
            <div class="relative group">
              <img src="@/assets/default_profile_image.png" class="w-10 h-10 rounded-full border-2 border-white"
                alt="User 1" />
              <div
            ...
            <div class="relative group cursor-pointer">
              <span class="flex items-center justify-center w-10 h-10 text-green-800 bg-green-100 rounded-full">+</span>
            </div>
          </div>
        </div>
        ...
        <!-- Close button -->
        <button @click="generalStore.closeModal()"
    ...
    " class="mt-4 inline-flex items-center px-4 py-2 text-black border border-transparent rounded-md font-semibold focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
        ...
        <button v-if="Object.keys(generalStore.filterParams).length" @click="
  ...
  <transition name="fade">
    <div v-if="generalStore.currentModal === 'aiChatBoxModal'"
  ...
  <!-- Assign Users Modal -->
  <div v-if="generalStore.currentModal === 'assignUserModal'"
    ...
    <div class="bg-white shadow-xl w-80 h-50 p-4">
      ...
      <div class="tags-input">
        ...
        <input type="text" v-model="searchQuery" @keydown.enter.prevent="addUserByInput" placeholder="Add a user" />
      </div>
      <!-- {{ projectStore.data.assignmentUsers }} -->
      <ul class="max-h-48 overflow-y-auto">
        <li class="px-4 py-2 cursor-pointer hover:bg-gray-100" v-for="(user, key) in projectStore.data.assignmentUsers"
          :key="key" @click="
